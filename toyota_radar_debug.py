...
class OnCan(can.Listener):
    def __init__(self):
        ...
        try:
            self.db = cantools.database.load_file(
                "opendbc/toyota_prius_2017_adas.dbc", strict=False
            )
    ...
    def on_message_received(self, msg):
        ...
        print(
            f"[RX] ID: 0x{msg.arbitration_id:03X} Data: {msg.data.hex().upper()} ({len(msg.data)} bytes)"
        )
            ...
            if self.db:
                try:
                    ...
                    if decoded.get("VALID") == 1:
                        self.valid_tracks += 1
                        print(
                            f"  *** VALID TRACK at dist: {decoded.get('LONG_DIST', 'N/A')} m ***"
                        )
                    else:
                        print(
                            f"  (Track message, but VALID={decoded.get('VALID', 'N/A')})"
                        )
                except Exception as e:
...
STATIC_MSGS = [
    (
        0x141,
        ECU.DSU,
        (CAR.PRIUS, CAR.RAV4H, CAR.LEXUS_RXH, CAR.RAV4, CAR.COROLLA),
        1,
        2,
        b"\x00\x00\x00\x46",
    ),
...
if __name__ == "__main__":
    ...
    try:
        can_bus1 = can.interface.Bus(
            interface="socketcan", channel="can1", bitrate=500000
        )
        can_bus2 = can.interface.Bus(
    ...
    try:
        db = cantools.database.load_file(
            "opendbc/toyota_prius_2017_pt_generated.dbc", strict=False
        )
    ...
    try:
        ...
        can_bus1.send(
            can.Message(
                arbitration_id=cruise_message.frame_id,
                data=active_message,
                is_extended_id=False,
            )
        )
        ...
        can_bus1.send(
            can.Message(
                arbitration_id=msg.frame_id,
                data=msg.encode(
                    {"MAIN_ON": 0, "CRUISE_CONTROL_STATE": 0, "UI_SET_SPEED": 0}
                ),
                is_extended_id=False,
            )
        )
        ...
        while True:
            ...
            if frame % 1 == 0:
                ...
                msg = can.Message(
                    arbitration_id=acc_message.frame_id,
                    data=acc_msg,
                    is_extended_id=False,
                )
                can_bus1.send(msg)
            ...
            for addr, ecu, cars, bus, fr_step, vl in STATIC_MSGS:
                if frame % fr_step == 0:
                    ...
                    message = can.Message(
                        arbitration_id=addr, data=bytes(tosend), is_extended_id=False
                    )
